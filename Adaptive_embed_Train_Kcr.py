...
if __name__ == '__main__':
	...
	for index_fold in all_index: #index_fold #test_index
		...
		#---------------------------------->>
		species = 'Kcr'
		train_pos_fa = './data/Kcr/Kcr_positive_train.fa'
		train_neg_fa = './data/Kcr/Kcr_negative_train.fa'
		
		OutputDir = './Result/Adapt_Train_Kcr/fold_{0}'.format(index_fold)
		...
		for epoch in range(EPOCH):
			...
			with torch.no_grad():
				for step, (valid_x, valid_y) in enumerate(val_loader):     #val_loader
					...
					# loss = criterion(y_hat_val, valid_y.to(device)).item()      # batch average loss		  #C
